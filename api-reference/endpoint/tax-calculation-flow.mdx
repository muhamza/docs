...
curl https://api.stripe.com/v1/checkout/sessions \
  ...
  -d "line_items[0][price]=price_1QDLJLLSmZDVqIUdhb06qBvD" \
...
**Note:**
- Replace **your-stripe-api-key-here** with your actual Stripe API key.
- The Tax Rate IDs used in the cURL, were generated in Step 2.
- The **sphere_tax_calculation_id** was provided by the Sphere Api, in Step 1.
 


**2. One-Time Payment Checkout Session**

A one-time payment checkout is used for a single transaction. After the payment is processed, no further charges occur unless a new payment session is created. It is typically used for one-off 
purchases or services.

For one-time payments, be sure to enable invoice creation by setting **-d "invoice_creation[enabled]=true"**. This links the sphere_tax_calculation_id to the 
resulting invoice for tracking.

Additionally, pass the **sphere_tax_calculation_id** as a custom field in the invoice to link it to the resulting Stripe invoice for accurate tracking.

Here’s how you should include the **sphere_tax_calculation_id** in your request:

**Sample cURL for One-Time Payment:**
```
curl https://api.stripe.com/v1/checkout/sessions \
  -u "your-stripe-api-key-here" \
  --data-urlencode "success_url=https://dashboard.staging.getsphere.com/monitoring/" \
  -d "line_items[0][price]=price_1QIVVnLSmZDVqIUdC5FeIsRn" \
  -d "line_items[0][quantity]=1" \
  -d "line_items[0][tax_rates][0]=txr_1QJqyzLSmZDVqIUdFs615ZB6" \
  -d "line_items[0][tax_rates][1]=txr_1QJqztLSmZDVqIUdlZ4NjC7l" \  
  -d "invoice_creation[enabled]=true" \
  -d "invoice_creation[invoice_data][custom_fields][0][name]=sphere_tax_calculation_id" \
  -d "invoice_creation[invoice_data][custom_fields][0][value]=a0bc2cce-e028-4922-ac7e-297f52c0e3e3" \
  -d "mode=payment"
```
...
## **<u>Step 3b: Create Direct Invoice Creation (One-Time Payment)</u>**
...
-u "your-stripe-api-key-here" \
-d "customer=cus_R5WZX8xXNZRwQh" \
-d "auto_advance=true" \
-d "collection_method=charge_automatically" \
-d "metadata[sphere_tax_calculation_id]=a0bc2cce-e028-4922-ac7e-297f52c0e3e3"
```
...
-u "your-stripe-api-key-here" \
-d "customer=cus_R5WZX8xXNZRwQh" \
-d "price=price_1QIVVnLSmZDVqIUdC5FeIsRn" \
-d "quantity=1" \
